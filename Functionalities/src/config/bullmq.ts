...
const redisConfig = {
  REDIS_HOST: 'valkey-385d397b-ogasominali-99f4.g.aivencloud.com',
  REDIS_PORT: 28688,
  REDIS_PASSWORD: 'AVNS_dJ4CjR16TQH5Ia2WdI2',
...
export const testRedisConnection = async (): Promise<boolean> => {
  logger.info('Testing Redis connection with the following settings:');
  logger.info(`- Host: ${redisConfig.REDIS_HOST}`);
  logger.info(`- Port: ${redisConfig.REDIS_PORT}`);
  logger.info(`- TLS Enabled: ${redisConfig.REDIS_TLS_ENABLED}`);
  logger.info(`- Using Username: ${redisConfig.REDIS_USERNAME}`);
  logger.info(`- Using Password: ${!!redisConfig.REDIS_PASSWORD}`);
  ...
  try {
    ...
    redis.on('error', (error) => {
      logger.error(`Redis error during connection test: ${error.message}`);
    });
    ...
    logger.info(`Retrieved test value from Redis: "${testResult}"`);
    
    await redis.quit();
    
    if (testResult === 'success') {
      logger.info('✅ Successfully connected to Redis and verified read/write access');
      return true;
    } else {
      logger.error('❌ Connected to Redis but test failed - unexpected return value');
      return false;
    }
  } catch (error) {
    ...
    try {
      ...
      logger.error('Error while closing Redis connection:', quitError);
    }
    return false;
  }
};
